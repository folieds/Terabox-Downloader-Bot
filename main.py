...
async def stats_command(m: UpdateNewMessage):
    if m.is_group or m.is_channel:
        return
    check_if = await is_user_on_chat(bot, f"@OutlawBots", m.peer_id)
    if not check_if:
        return await m.reply(f"Please join @OutlawBots then send me the link again.")
    check_if = await is_user_on_chat(bot, f"@OutlawBots", m.peer_id)
    if not check_if:
        return await m.reply(f"Please join @OutlawBots then send me the link again.")
...
"""
    ...
    await m.reply(
        ...
        buttons=[
            [
                Button.url("Update Channel", "https://t.me/OutlawBots"),
                Button.url(
                    "Repo", "https://t.me/outlawbots"
                ),
            ]
        ],
        link_preview=False,
...
"""
    ...
    await m.reply(
        ...
        buttons=[
            [
                Button.url("Updates", "https://t.me/OutlawBots"),
                Button.url(
...
async def handle_message(m: Message):
    ...
    check_if = await is_user_on_chat(bot, "@OutlawBots", m.peer_id)
    if not check_if:
        return await m.reply("Please join @OutlawBots  then send me the link again.")
    check_if = await is_user_on_chat(bot, "@OutlawBots", m.peer_id)
    if not check_if:
        return await m.reply("Please join @OutlawBots then send me the link again.")
    is_spam = db.get(m.sender_id)
    if is_spam and m.sender_id not in [5767166269]:
